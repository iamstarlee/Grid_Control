...
from model import CEEMDANCNNBiLSTMModel
from prun import *
...
# 参数与配置
seed = 18  # 设置随机种子，以使实验结果具有可重复性
def set_deterministic(seed=18):
    random.seed(seed)
    np.random.seed(seed)
...
log_dir = "logs/" + current_time +  f'_{data_name}' + f'_{n}_{m}_{prun_start}_{prun_end}'
...
def model_train(batch_size, epochs, model, optimizer, loss_function, train_loader, val_loader, test_loader):
    ...
    # 模型评估,测试集
    scaler = load(f'data_ed/{data_name}_720_4_scaler')
